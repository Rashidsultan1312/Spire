...
const PlatTex := preload("res://assets/sprites/v3/tiles/platform.png")
const WallTex1 := preload("res://assets/sprites/v3/tiles/wall1.png")
const WallTex2 := preload("res://assets/sprites/v3/tiles/wall2.png")
const DECOR_SHIELD := "res://assets/sprites/v3/tiles/torch.png"
const DECOR_SWORDS := "res://assets/sprites/v3/tiles/torch.png"
const DECOR_PAINTING := "res://assets/sprites/v3/tiles/torch.png"
const DECOR_AXES := "res://assets/sprites/v3/tiles/torch.png"
...
func setup(level_idx: int, theme: Dictionary = {}):
	...
	var key_chance := 30 if km_lvl >= 3 else 20
	for i in range(tiles.size()):
		if tiles[i].tile_type == 0:
			if randi() % 100 < key_chance:
...
func _spawn_tiles(n: int):
	...
	for i in range(n):
		...
		_spawn_tile_pedestal(tile)

func _spawn_tile_pedestal(tile: Area2D):
	var pedestal := Sprite2D.new()
	pedestal.texture = WallTex1
	pedestal.texture_repeat = CanvasItem.TEXTURE_REPEAT_ENABLED
	pedestal.texture_filter = CanvasItem.TEXTURE_FILTER_NEAREST
	pedestal.region_enabled = true
	pedestal.region_rect = Rect2(0, 0, 80, 24)
	pedestal.scale = Vector2(1.5, 1.5)
	pedestal.position = Vector2(0, 85)
	pedestal.z_index = -2
	pedestal.modulate = Color(0.85, 0.8, 0.9, 0.9)
	tile.add_child(pedestal)
...
func _spawn_platforms():
	...
	stone.region_rect = Rect2(0, 0, 190, 64)
	...
	wood.region_rect = Rect2(0, 0, 190, 14)
