...
@onready var diff_left: Button = $Panel/VBox/DiffRow/DiffLeft
@onready var diff_right: Button = $Panel/VBox/DiffRow/DiffRight
...
const LockTex := preload("res://assets/sprites/v3/ui/lock.png")
const PreGameScene := preload("res://scenes/pre_game.tscn")

var tile_panels: Array[PanelContainer] = []
var pre_game: CanvasLayer
...
func _start():
	SoundManager.play("click")
	var has_items := false
	for count in GameManager.items.values():
		if count > 0:
			has_items = true
			break
	if has_items:
...
func _show_pre_game():
	if pre_game and is_instance_valid(pre_game):
		pre_game.queue_free()
	pre_game = PreGameScene.instantiate()
	get_parent().add_child(pre_game)
	pre_game.start_pressed.connect(func(): GameManager.start_game())
	pre_game.skip_pressed.connect(func(): GameManager.selected_power_ups.clear(); GameManager.start_game())
	var tw := create_tween()
	tw.tween_property(panel, "modulate:a", 0.0, 0.15)
	tw.tween_callback(func(): pre_game.open())
